# Chrome DevTools MCP 設定與操作說明：Codex VSCode Extension 版

適用課程：第三階段「前端整合、網頁檢查與部署測試」

## 1. 這是什麼？

Chrome DevTools MCP 是 Google Chrome DevTools 團隊提供的 MCP server。設定完成後，VSCode 裡的 Codex extension 可以透過它操作真實 Chrome 瀏覽器，協助檢查網頁、找錯誤、測試互動與改善畫面。
...
## 4. 安裝 Codex VSCode Extension

1. 開啟 VSCode。
2. 點左側 Extensions。
3. 搜尋 `Codex`。
4. 安裝 OpenAI 提供的 Codex extension。
5. 依照畫面指示登入或授權。
6. 重新載入 VSCode。
...
```text
VSCode 側邊欄或 Chat 區域可以看到 Codex 對話入口。
```
...
如果你的 VSCode 或 Codex extension 支援 MCP 設定檔，可以加入以下設定：
...
設定完成後，重新啟動 VSCode，讓 Codex extension 載入 MCP server。
...
重新啟動 VSCode 後，打開 Codex extension 對話區，輸入：

```text
請使用 Chrome DevTools MCP 檢查 https://developers.chrome.com 的效能。
```

成功時，Codex 會透過 MCP 啟動 Chrome，打開網站，並進行檢查。

注意：Chrome 不一定會在 VSCode 啟動時立刻打開，通常是等到 Codex 真的需要使用瀏覽器工具時才啟動。
...
- Codex extension 支援 MCP 並已載入 MCP server
...
這是正常的。Chrome 通常會在 Codex 實際執行「開網頁、截圖、檢查效能」等工具時才啟動。
...
```text
Chrome DevTools MCP 讓 VSCode 裡的 Codex 真的打開網頁、操作頁面、看錯誤訊息。
它適合用在前端成果檢查與修正。
```
